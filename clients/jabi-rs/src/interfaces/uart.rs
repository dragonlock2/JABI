use crate::interfaces::{Interface, InterfaceRequest, REQ_PAYLOAD_MAX_SIZE};
...
pub fn get_device(port: &str, baud: usize) -> Result<Device, serial::Error> {
    ...
    let mut iface = UARTInterface {
        ...
        max_req_size: REQ_PAYLOAD_MAX_SIZE,
    };
    ...
    match dev.req_max_size() {
        Ok(s) if s >= REQ_PAYLOAD_MAX_SIZE => {
            dev.iface.lock().unwrap().set_max_req_size(s);
            Ok(dev)
        }
